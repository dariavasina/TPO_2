...
def test_generate_csv_content():
    ...
    with open(TEST_FILENAME, mode="r") as csvfile:
        ...
        assert x_values == [0.0, 0.5, 1.0], f"Expected x values [0.0, 0.5, 1.0], got {x_values}"
...
def test_generate_csv_with_custom_delimiter():
    ...
    with open(TEST_FILENAME, mode="r") as file:
        content = file.read()
        assert delimiter in content, f"Custom delimiter '{delimiter}' not found in CSV content"


@patch('functions.csv_generator.system_function')
def test_generate_csv_calls_system_function(mock_system_function):
    mock_system_function.return_value = 52
    
    start, end, step = 0, 2, 1
    generate_csv(start, end, step, TEST_FILENAME)
    
    assert mock_system_function.call_count == 3
    mock_system_function.assert_any_call(0, EPSILON)
    mock_system_function.assert_any_call(1, EPSILON)
    mock_system_function.assert_any_call(2, EPSILON)
...
def test_generate_csv_with_zero_step():
    with pytest.raises(Exception):
        generate_csv(0, 10, 0, TEST_FILENAME)
